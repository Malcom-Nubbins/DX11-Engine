...
BasicLight::BasicLight(SystemHandlers* systemHandlers)
...
HRESULT BasicLight::InitialiseShaders()
{
	...
	hr = _systemHandlers->GetShaderClass()->CreateVertexShader((WCHAR*)L"Core/Shaders/LightVS.hlsl", &_lightVS, &_simpleVertexInputLayout, layout, ARRAYSIZE(layout));
	if (FAILED(hr))
		return hr;
